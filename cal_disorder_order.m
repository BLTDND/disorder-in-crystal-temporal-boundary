function [t_ratio,E_t_stor,E_omiga_2]=cal_disorder_order(V_before,V_after,disorder_C_before,disorder_C_after,gamma_c0,disorder_gamma,random_value)
...
for n_trip=1:N_t 
    ...
    for tt=1:N_in_trip
        ...
        t=n_t*delta_t;
        if n_trip<=V_before
            V_t(n_t)=(V+disorder_C_before*random_value(1,tt))*cos(Omega*t);%V_t(n_t)=V*cos(Omega*t);
        else
            V_t(n_t)=(V+disorder_C_after*random_value(1,tt))*cos(Omega*t);%V_t(n_t)=V*cos(Omega*t);
        end
        % for ring A&B z+pm
        if tt>1
